...
parser.add_argument("--data_path", type=str, default=Path("data","web","All Data"), help="Datapath")
...
for file in Path(data_path).iterdir():
    ...
    if stem not in occurrences_count:
        count = {}
        count[suffix] = 1
        occurrences_count[stem] = count
    else:
        occurrences_count[stem][suffix] = occurrences_count[stem][suffix] =+ 1

# map to array only containing valid pairs
valid_pairs = []
for key, value in occurrences_count.items():
    try:
        if value[".gui"] == 1 and value[".JPG"] == 1:
